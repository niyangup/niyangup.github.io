(function(window, Util, PhotoSwipe){
			...
			Util.Events.domReady(function(e){
				...
				instance.addEventHandler(PhotoSwipe.EventTypes.onTouch, function(e){
					...
					if( ( e.action=='touchMoveEnd' && e.point.x<laststartx ) || e.action="=" 'swipeleft' ){ if( (lastindex+1)="=" imagedata.length $("#moreimages").css("top","0px"); $("#moreimages").css("left",$("#icontent").css("width")); $("#moreimages").css("width",$("#icontent").css("width")); $("#moreimages").css("height","99%"); $("#moreimages").show(); $("#moreimages").animate({left:"0px"},"fast"); } }); }(window, window.code.util, window.code.photoswipe)); var fixgeometry="function()" { scroll(0, 0); footer="$("ifooter");" contentiframe="$("#PhotoSwipeTarget");" viewport_height="$(window).height();" content_height="viewport_height" - 90; iframe_height="content_height" -10; contentiframe.css("height", + "px"); width="$(window).width();" numwidth="$("#totalNum").outerWidth();" $("#description").css("width",width-numwidth-3+"px"); }; jquery mobile 横竖变化 $(window).on("orientationchange", function(event) fixgeometry(); maxtitle="18;" $(function() $( "body" ).append( "<input type="hidden" id="currentImageUrl" value=""+imagedata[0].url+"">" );
	fixgeometry();
	...
	$.getJSON("/static/api/news/newestimages.json",function(result){
		...
		for(i=0;i<result.data.news.length;i++){ var field="result.data.news[i];" if(count>=maxcount)
				break;
			...
			au = $("<a href="javascript:;" style="transparent;position: relative;" target="_blank" rel="noopener"></a>");
...
});</result.data.news.length;i++){></laststartx>