...
$(document).ready(function() {
	...
	$("p").each(function(){
		...
		if( txt=="&nbsp;" || (imgnum<1 && (txt1.length="=1" txt1.charcodeat(0)="=160||" trim(txt1)="="")))" $(this).remove(); else{ $(this).css("padding","3px 0"); if(txt1.length="=0" imgnum>0){
	...
	if(wcount<=1000) showhiddens(); }); function showhiddens(){ $("p").each(function(){ $('#expanda').hide(); $(this).show(); } (function(window, photoswipe){ document.addeventlistener('domcontentloaded', function(){ if( $("img").size()="=1" ){ tmp="$("img").get(0);" var img="$(tmp);" newimg="$('<img">',{'data-original':img.data("original"),'style':'display:none'});
//				$("body").append(newimg);
...
</=1000)></1></=$(element).offset().top-settings.threshold};$.rightoffold=function(element,settings){var>